// Animated quatrefoil bloom field used as a decorative background.
// Same visual identity as the marketing hero — reused on the sign-in page
// (and anywhere else we want the "logo meadow" feel).
...
type LogoMeadowProps = {
  // Mask that dims the center so foreground content reads clearly.
  // Marketing hero uses a wider mask; sign-in card needs a tighter mask.
  mask?: "hero" | "card" | "none";
};

const MASKS: Record<"hero" | "card" | "none", string | undefined> = {
  hero:
    "radial-gradient(ellipse 52% 42% at center, rgba(0,0,0,0.15), rgba(0,0,0,0.15) 28%, rgba(0,0,0,1) 78%)",
  card:
    "radial-gradient(ellipse 38% 32% at center, rgba(0,0,0,0.1), rgba(0,0,0,0.1) 22%, rgba(0,0,0,1) 72%)",
  none: undefined,
};

export function LogoMeadow({ mask = "hero" }: LogoMeadowProps) {
